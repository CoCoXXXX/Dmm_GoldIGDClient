...
{
    public class NetworkLoginGateServerState : StateAdapter<IAppContext>
    {
        private const string Tag = "NetworkLoginGateServerState";

        public override int GetStateCode()
        {
            return NetworkState.LoginGateServer;
        }
        ...
        {
            MyLog.DebugWithFrame(Tag, "Finish login gate server.");
            ...
            if (_isTimeout)
            {
                stateResult.ErrMsg = string.Format("登录超时，请重新登陆\n【错误码{0}】",
                    NetworkStateErrorCode.LoginGateServerFailCode);
            }
            else
            {
                stateResult.Result = StateResult.Ok;
                stateResult.NextStateCode = NetworkState.ConnectHallServer;
            }
            return stateResult;
        }
