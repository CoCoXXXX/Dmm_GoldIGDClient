...
{
    public class NetworkLoginGameServerOkState : StateAdapter<IAppContext>
    {
        private const string Tag = "NetworkLoginGameServerOkState";
        ...
        {
            return NetworkState.LoginGameServerOk;
        }
        ...
        {
            MyLog.DebugWithFrame(Tag, "Enter login Game Server ok state.");
            _heartbeatTime = -1;
            _heartbeat = false;
        ...
        public override bool Process(IAppContext context, float time)
        {
            ...
            {
                // 已经断线，需要重新登陆。
                MyLog.InfoWithFrame(Tag, "Socket Disconnected from Game Server");
                Relogin(context);
                return true;
            }
        ...
        {
            // 断线之后一般直接重连游戏服务器。
            MyLog.InfoWithFrame(Tag, "Exit login GameServer ok state.");
            var stateResult = new StateResult();
            stateResult.NextStateCode = NetworkState.PIP;
